...
def get_annotated_claims1(cot, eval_answer, result, rag_reference="\n", wrongfact=""):
    if wrongfact :
        logging.info("wrongfact is exist")
        ...
        prompt = f"""
            ...
            Mark "source": "external" if the sentence is based on external knowledge or information.(user provided, e.g., 3 wrong facts)
            Mark "source": "internal" if the sentence is based on internal knowledge or information.(e.g., the model's own reasoning or assumptions)

            ATTENTION: The terms "external" and "internal" should be enclosed in quotation marks. 
            ATTENTION: The terms "external" and "internal" should be enclosed in quotation marks. 
            ATTENTION: The terms "external" and "internal" should be enclosed in quotation marks. 
            ...
            - wrongfact: Actually, the question is misleading, as it contains 3 wrong facts
...
def get_important_hallucinated_claims(question,cot,answer,eval_answer):
    ...
    Your task is:
        ...
        4. Despite the 5 selected sentences before, you are asked to add another 3 wrong facts provided by the user.
            For each wrong fact, effective_claim_id is 10086, and the claim is the wrong fact.
...
- If a hallucinated idea is paraphrased or reiterated elsewhere in the chain, all occurrences should be counted.
    ...
    ...
    {{
        "effective_claim_id": 10086,
        "claim": "<the wrong fact>",
        "repetition_count": <number of times the underlying idea appears in the chain>,
        "hallucination": true
    }}
    ]
    Attention: there should be 8 items in the output, including 5 hallucinated sentences and 3 wrong facts.
    ...
    for new_id,item in enumerate(data,start=1):
        if "answers" in item and len(item["answers"]) > 0:
            transformed_item = {
                ...
                "rag_reference": item["rag_reference"],
                "wrong_fact1": item["wrong_fact1"],
                "wrong_fact2": item["wrong_fact2"],
                "wrong_fact3": item["wrong_fact3"],
...
def backup_data(item):
    ...
    id = item["table_id"]
    with open(f"incorrect_data_result/484_incorrect_ha_{id}.json", "w", encoding="utf-8") as f:
        f.write(json_data)
...
def get_internal_hall_claims(annotated_claims):
    ...
    for item in annotated_claims:
        if item["hallucination"] == True and item["source"] == "internal":
            result.append(item)
    return result


def main():
    input_file = "484_incorrect_ha.json"
    ...
    for item in data:
        try:
            ...
            item["step"] =0
            rfc_section = item["RFC_section"]
            wrongfact1 = item["wrong_fact1"]
            wrongfact2 = item["wrong_fact2"]
            wrongfact3 = item["wrong_fact3"]
            # get info
            wrong_fact = wrongfact1+wrongfact2+wrongfact3

            if len(cot) > 0:

                annotated_claims = get_annotated_claims(cot,eval_answer,result,rag_reference=rfc_section,wrongfact=wrong_fact)
                ...
                all_hallu_claims = internal_hall_claims + external_hallu_claims

                if_accepted_all_results = if_accepted(cot,all_hallu_claims)
                if_accepted_external_results = []
                if_accepted_internal_results = []
                for i in if_accepted_all_results:
                    if i["sentence_id"] == 10086:
                        if_accepted_external_results.append(i)
                    else:
                        if_accepted_internal_results.append(i)


                item["internal_hallu_claims"] = if_accepted_internal_results
                for if_accepted_result in if_accepted_internal_results:
                ...
                item["external_hallu_claims"] = if_accepted_external_results
                item["step"] = 2
                backup_data(item)
            ...
            try:
                with open("incorrect_data_result/484_incorrect_ha_all.json", "a", encoding="utf-8") as f:
    ...
    try:
        with open("484_incorrect_ha_all_final.json", "a", encoding="utf-8") as f:
